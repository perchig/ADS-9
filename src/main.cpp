...
int main() {
  ...
  for (int n = 3; n <= 9; ++n) {
    ...
    auto getAllPerms_time = std::chrono::duration_cast<
        std::chrono::microseconds>(end - start).count();
    start = std::chrono::high_resolution_clock::now();
    std::vector<char> perm1 = getPerm1(testTree, randomPerm);
    end = std::chrono::high_resolution_clock::now();
    auto getPerm1_time = std::chrono::duration_cast<
        std::chrono::microseconds>(end - start).count();
    start = std::chrono::high_resolution_clock::now();
    std::vector<char> perm2 = getPerm2(testTree, randomPerm);
    end = std::chrono::high_resolution_clock::now();
    auto getPerm2_time = std::chrono::duration_cast<
        std::chrono::microseconds>(end - start).count();
    dataFile << n << "," << getAllPerms_time << "," << getPerm1_time
             << "," << getPerm2_time << "\n";
  }
  dataFile.close();
  return 0;
